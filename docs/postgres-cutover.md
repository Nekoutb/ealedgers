...
Security policies shipped in migration `0006_postgres_rls`.
...
- An off-host backup of `db.sqlite3` already exists (Apache vhost or
  rsync target — anywhere not on the same disk).
...
## 3. Add psycopg + bump deps on the server
...
grep -c '"model": "accounting.tenant"' /tmp/ealedgers-dump.json    # 1 (or more)
```
...
This runs migrations `0001` through `0006` against the empty Postgres
database. Migration `0006` installs the RLS policies.
